...
아래 두 다이어그램은 그 판단의 앞뒤입니다. 먼저 병목의 시작점인 유입 구간이 어디에 묶여 있었는지 보이고, 다음으로 그 구간만 떼어내 격리한 구조를 보입니다.
...
- 응답과 저장의 분리 — 스캔 요청을 받으면 Kafka로 넘기고 바로 응답합니다. 응답 시간이 DB 커밋을 기다리지 않습니다.
- 유입 속도와 저장 속도의 분리 — Redis Streams를 DB 앞에 두고, 워커가 감당할 만큼만 꺼내 가게 했습니다.
- 건별 INSERT의 배치 전환 — JdbcTemplate.batchUpdate와 rewriteBatchedStatements로 여러 건을 다중 값 INSERT 한 문장으로 보냅니다.
- 자원의 분리 — 유입과 저장을 별도 애플리케이션으로 나눠, 스레드 풀과 커넥션 풀이 서로 다른 JVM에 놓이게 했습니다.
- 소비의 분산 — 저장 워커를 여러 인스턴스로 띄워, 한 인스턴스가 막혀도 다른 인스턴스가 소비를 이어갑니다.
...
개선이 실제로 효과가 있었는지 두 관점으로 나눠 확인했습니다. 하나는 평상시 물량에서 응답 시간과 DB 쓰기 부하가 줄었는지이고, 다른 하나는 한 구간이 막혔을 때 그 여파가 다른 구간으로 번지지 않는지입니다. 기존 동기 구조와 재설계한 비동기 구조를 같은 조건의 컨테이너에 올려 같은 부하를 흘렸습니다.

**장애 조건을 이렇게 정한 이유**

당시 장애는 지표가 먼저 알려주지 않았습니다. 물류 센터는 주문 마감과 출고 물량이 움직이는 시간대에 서버 지표가 전반적으로 치솟기 때문에, DB 락이나 메모리 부족처럼 시스템을 멈추는 오류가 아니면 모니터링만으로 원인을 짚어낼 수 없었습니다. 장애 인지는 센터 직원이 원격 접속을 수락해 주고, 개발자가 스캐너 PC의 로컬 MySQL에서 스캔 상태가 신규에 멈춘 채 쌓여 있는 것을 확인하는 순서였습니다. 4시간 이상 누적된 수천 건 규모였습니다.

원인은 여러 지표를 겹쳐 놓고서야 판단할 수 있었습니다. 스캐너 PC의 코드가 응답을 기다리는 동기 구조였고, 중앙 MySQL의 커넥션 사용이 상한에 닿아 대기가 발생했으며, 스캔 이후 서비스 로직의 트레이스 지연이 가파르게 올라갔습니다. 같은 시간대에 고객사의 주문 엑셀 업로드가 겹쳤는데, 이 처리가 파일 전체를 한 번에 메모리로 읽는 방식이라 한 컨테이너의 메모리를 대부분 점유했습니다. 모놀리스에 동기 구조가 얹혀 있으니, 한 곳의 자원 점유가 스캔 처리까지 그대로 번진 것입니다.

그래서 검증의 초점을 물량 크기가 아니라 전파 경로에 두었습니다. 저장 계층의 커넥션 풀을 포화시켜 그 여파가 어디까지 가는지를 봅니다. 아래 동시성 값은 당시 물량을 흉내 낸 것이 아니라, 이 경로가 압력에 따라 뚜렷해지는지 확인하려고 단계적으로 올린 값입니다.

**부하 조건**

| 구분 | 평시 부하 | 장애 부하 |
| :--- | :--- | :--- |
| 목적 | 응답 시간과 DB 쓰기 부하 비교 | 저장 계층 포화가 어디까지 번지는지 관측 |
| 부하 대상 | 스캔 경로(`/scan/barcode`, `/api/barcode`) | `touch-recent` API로 저장 계층 커넥션 풀을 직접 포화 |
| 동시 요청 | 10 스레드 | 30 → 50 → 80 단계 상승 |
| 램프업 | 30초 | 없음 |
| 지속 시간 | 180초 + 120초 | 60초(격리 확인), 180초(전파 확인) |
| 목표 처리량 | 초당 100건 | 제한 없음, 커넥션 풀이 찰 때까지 |

장애 부하의 동시성이 신규 80, 구형 30으로 다른 것은 커넥션 풀 크기가 각각 20과 10이라 포화에 필요한 압력이 달랐기 때문입니다. 두 경우 모두 풀이 100% 찬 상태를 만든 뒤 비교했습니다.

> 병목이 눈에 보이도록 전체 규모를 축소해 재현했습니다.

<br>

## 검증 1 — 평시 부하에서의 응답 시간과 DB 쓰기 부하
