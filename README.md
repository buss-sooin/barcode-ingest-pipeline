...
프로모션이나 명절과 같이 물량이 몰리는 날이면, 대량의 바코드 스캔이 한꺼번에 유입되면서 바코드 처리와 저장에 병목이 발생했습니다. 현장에서는 최악의 경우 4시간 이상 바코드 스캔 결과가 처리되지 않아, 새벽 배송의 실질적인 업무에 큰 차질을 빚었습니다.

장애는 지표가 먼저 알려주지 않았습니다. 마감과 출고 물량이 움직이는 시간대에는 서버 지표가 전반적으로 치솟기 때문에, 센터 직원의 원격 접속 수락을 받아 스캐너 PC의 로컬 MySQL에서 스캔 상태가 신규에 멈춘 채 쌓인 것을 확인하고서야 병목을 인지할 수 있었습니다. 원인은 여러 지표를 겹쳐 놓고 판단했습니다. 스캐너 PC의 코드가 응답을 기다리는 동기 구조였고, 중앙 MySQL의 커넥션 사용이 상한에 닿아 대기가 발생했으며, 스캔 이후 서비스 로직의 트레이스 지연이 가파르게 올라갔습니다. 같은 시간대에 겹친 고객사의 주문 엑셀 업로드는 파일 전체를 한 번에 메모리로 읽는 방식이라 한 컨테이너의 메모리를 대부분 점유했습니다. 모놀리스에 동기 구조가 얹혀 있으니, 한 곳의 자원 점유가 스캔 처리까지 그대로 번진 것입니다.
...
- 부하 대상: 스캔 경로(`/scan/barcode`, `/api/barcode`)
...
- 부하 대상: `touch-recent` API로 저장 계층 커넥션 풀을 직접 포화
