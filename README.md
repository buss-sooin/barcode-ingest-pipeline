...
장애는 지표가 먼저 알려주지 않았습니다. 물류 센터는 주문 마감과 출고 물량이 움직이는 시간대에 서버 지표가 전반적으로 치솟기 때문에, DB 락이나 메모리 부족처럼 시스템을 멈추는 오류가 아니면 모니터링만으로 원인을 짚어낼 수 없었습니다. 인지는 센터 직원이 원격 접속을 수락해 주고, 개발자가 스캐너 PC의 로컬 MySQL에서 스캔 상태가 신규에 멈춘 채 쌓여 있는 것을 확인하는 순서였습니다.

원인은 여러 지표를 겹쳐 놓고서야 판단할 수 있었습니다. 스캐너 PC의 코드가 응답을 기다리는 동기 구조였고, 중앙 MySQL의 커넥션 사용이 상한에 닿아 대기가 발생했으며, 스캔 이후 서비스 로직의 트레이스 지연이 가파르게 올라갔습니다. 같은 시간대에 겹친 고객사의 주문 엑셀 업로드는 파일 전체를 한 번에 메모리로 읽는 방식이라 한 컨테이너의 메모리를 대부분 점유했습니다. 모놀리스에 동기 구조가 얹혀 있으니, 한 곳의 자원 점유가 스캔 처리까지 그대로 번진 것입니다.

> 원활한 성능 비교를 위해 병목이 관측 가능한 수준의 스케일로 축소함

<br>

## 관점 1 — 평상시 물량의 성능 개선 효과
