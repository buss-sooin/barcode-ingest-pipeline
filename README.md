...
worker-1의 커넥션 풀이 60초간 한계에 머무는 장애 구간(worker-2 풀은 내내 여유). 아래 두 그래프는 같은 구간에서 이 장애가 어디까지 번지는지 보여줌. (급격한 물량 인입)

![신규 — 워커 포화 구간에도 유입 평균 응답시간 무영향](docs/images/ingest-latency-isolated.png)
같은 구간의 ingest 스캔 처리 평균 응답시간(서버 측정). 포화 내내 18~22ms 범위에서 상승 없이 유지 — 저장 계층의 포화가 유입 계층으로 번지지 않음. (급격한 물량 인입)
