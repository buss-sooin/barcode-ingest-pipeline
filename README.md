...
이 검증은 프로세스가 재시작되며 디스크는 그대로 유지되는 경우를 전제로 합니다. 컨테이너·인스턴스 자체가 교체되는 경우까지 포함하는 인프라 계층의 백업·복구 전략은 이 프로젝트 규모에 맞춰 상세히 설계하지 않았습니다 — 실제 운영 환경으로 확장한다면 영속 볼륨 구성이 추가로 필요하다는 점을 인지하고 있습니다.
