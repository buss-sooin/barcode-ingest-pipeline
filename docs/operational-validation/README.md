...
`BIP-FR-005-RC-R1`은 `FROZEN` 상태다. Historical Run #1 `BIP-FR-005-MR-20260909T124024Z`와 Historical Run #2 `BIP-FR-005-MR-20260911T112314Z`는 모두 `INCONCLUSIVE / NOT_REUSABLE`로 보존한다. 무효 Run #2의 `FS-01/FS-02` 관측은 Verified Reproduction Claim이 아니다. Successor Run `BIP-FR-005-MR-20260915T031000Z`은 baseline isolation V1 reconciliation, registration eligibility와 static preflight를 통과해 `REGISTERED / NOT_EXECUTED / NOT_ASSIGNED` 상태다. Implementation `83eaa799...`의 frozen image `sha256:7c88b617...`를 no-build 배포했으나 runtime preflight가 `bip-fr-002-broker-2` exit 137/OOM과 9개 under-replicated partition을 발견해 `BLOCKED`됐다. Material Run은 시작하지 않았다. Historical DLT는 삭제하지 않고 partition watermark, exact record ownership과 successor root-identity absence로 격리하며 unknown residue·watermark drift·identity collision은 fail closed로 차단한다.
