...
`BIP-FR-005-RC-R1`은 `FROZEN` 상태다. Historical Run #1 `BIP-FR-005-MR-20260909T124024Z`는 정상 영속화 기준선 실패로 Redis 장애 주입 전에 중단됐으며 `ABORTED / SUPERSEDED / NOT_REUSABLE`, Outcome `INCONCLUSIVE`로 보존한다. Historical Run #2 `BIP-FR-005-MR-20260911T112314Z`는 pre-fault collection이 `broker-3` OOM을 관측했지만 fault transition을 차단하지 못한 실행 제어 편차로 `ABORTED / NOT_REUSABLE`, Experiment Validity `FAIL`, Outcome `INCONCLUSIVE`다. 무효 Run의 `FS-01/FS-02` 관측은 Verified Reproduction Claim이 아니다. Successor Run `BIP-FR-005-MR-20260915T031000Z`은 baseline isolation V1 reconciliation을 통과해 `REGISTERED / NOT_EXECUTED / NOT_ASSIGNED` 상태이며, 새 frozen release와 entry preflight가 아직 남아 있다. Historical DLT는 삭제하지 않고 partition watermark, exact record ownership과 successor root-identity absence로 격리하며 unknown residue·watermark drift·identity collision은 fail closed로 차단한다. Record/Git은 registration 이후의 synchronization이며 실패 시 실행은 `SYNCHRONIZATION_BLOCKED`다.
