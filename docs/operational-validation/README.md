...
| BIP-FR-005 — Redis Streams Unavailability | Active traffic 중 Redis handoff failure, source retry, DLT와 bounded disposition 검증 예정 | Historical Run 2건 `INCONCLUSIVE` / Successor Run `REGISTERED / NOT_EXECUTED` | [Reproduction Record](./failure-reproduction/BIP-FR-005-redis-streams-unavailability/REPRODUCTION-RECORD.md) |
...
`BIP-FR-005-RC-R1`은 `FROZEN` 상태다. Historical Run #1 `BIP-FR-005-MR-20260909T124024Z`와 Historical Run #2 `BIP-FR-005-MR-20260911T112314Z`는 모두 `INCONCLUSIVE / NOT_REUSABLE`로 보존한다. 무효 Run #2의 `FS-01/FS-02` 관측은 Verified Reproduction Claim이 아니다. Successor Run `BIP-FR-005-MR-20260915T031000Z`은 baseline isolation V1 reconciliation과 registration eligibility를 통과해 `REGISTERED / NOT_EXECUTED / NOT_ASSIGNED` 상태다. Implementation `83eaa799...`의 frozen image `sha256:7c88b617...`를 no-build 배포했다. 최초 runtime preflight가 `bip-fr-002-broker-2` exit 137/OOM과 9개 under-replicated partition을 발견해 차단했으나, 구성 변경 없이 기존 broker-2를 시작한 뒤 617초/15개 표본의 확장 안정성 검증에서 OOM 재발과 예상 밖 restart가 없었고 Kafka URP/unavailable partition `0`, full ISR `PASS`를 유지했다. 등록 baseline 무결성과 static/runtime preflight가 모두 `PASS`이므로 현재 Run은 실행 자격을 회복했지만 아직 시작하지 않았다. 가장 강하게 뒷받침되는 용량 진단은 `DOCKER_HOST_MEMORY_PRESSURE`이며 kernel 수준 OOM 인과관계는 확정되지 않았고 잔여 용량 위험은 존재한다. Historical DLT는 삭제하지 않고 partition watermark, exact record ownership과 successor root-identity absence로 격리하며 unknown residue·watermark drift·identity collision은 fail closed로 차단한다.
